...
21. **Canonical Zones 9–15**: `vesicles`, `lysosomes`, `peroxisomes`, `vacuole`, `nuclear-pores`, `membrane-receptors`, `dna` — NOT cell-wall/chromosomes/centrioles. Source: Description §6 zone taxonomy.
22. **BP13 zone assignments**: BP13 (LLPS condensate, σ=0.75) belongs in Zone 3 (cytoplasm, cgroup/NUMA metaphor) AND Zone 5 (nucleus, BMAL1 LLPS phase). Combined σ unchanged: 0.90 for cytoplasm (BP2 dominant), 0.88 for nucleus (BP12 dominant).
23. **PlasmaLiteralness type coverage**: electrolyte-analogy = BP2, BP3, BP7, BP12, BP14. field-coherence-analogy = BP4, BP5, BP6, BP8, BP9, BP10, BP13. "reserved" is a status override, not a σ threshold — BP8 at σ=0.32 is reserved because it's zero-weight by design, not because σ < 0.30.
24. **BIOPLASMA §13 sync rule**: After any SPA phase completes, mark the corresponding §13 actions as ✅ IMPLEMENTED. Actions 1–4, 8, 9 done (June 2026). Actions 10 (BP12 circadian hook) and 11 (BP14 calcium spark hook) remain pending runtime TypeScript implementation.
25. **BP12/BP14 §8.6 status**: Both are constants-only with `isMetaphor:true`. §8.6 status must say "⏸ Constant exported; isMetaphor:true; no runtime hook/UI; pending Action 10/11" — never "✅ IMPLEMENTED_BIOPLASMA_PATHWAYS". IMPLEMENTED_BIOPLASMA_PATHWAYS excludes BP6, BP8, BP12, BP13, BP14.
26. **§7.4 powerhint.xml canonical zone and BP**: Zone = mitochondria / cytoskeleton; BP Enhancement = BP6 (Fröhlich pump) + BP1 (IRQ ground-state). NOT cytoplasm, NOT BP2. Section title = "Metabolic Coherence Tuning" not "Cytoplasmic Flux Optimisation".
27. **CellOsDomain.kt deployment targets**: Per ROM plan §3d line 164, both CellOsDomain.kt and cell_os_domain.json are committed to BOTH `org.cellos.cellshell/assets/` AND `CellVitalService/generated/`. Both targets must appear in §11.1 Manifold row and Description §13 row.
