...
**How to apply**: Before writing or reviewing any LineageOS doc, grep for `addThermalStatusChangedListener`, `smem_coherence.c`, `ThermalManager`, `lineage/interfaces/thermal`, `lineage/interfaces/performance`, `lineage-build.prop`, `IWaterCoherence`, `org.cellos.cellshell/Security`, `Trust`, `android_kernel_qcom_sm7325`, `AIDL HAL callback` (in P8 context), `seven biophoton`, `P1-P7` (in gen-2 manifold context), `0\.65` near BP4, `nucleus` near CellVitalService — all are wrong or superseded. Always verify σ values against BIOPLASMA_RESEARCH.md and BIOPHOTON_RESEARCH.md, not the Manifold summary tables. Zone matrix σ values must be kept in sync with §1.2 and authority docs after every correction pass.
