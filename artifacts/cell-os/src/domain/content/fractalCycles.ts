...

  {
    ...
    phases: [
      ...
      },
      {
        ...
        description:
          "The cytoskeleton's affect operates through three distinct filament systems, each with its own dynamics. (1) Actin polymerization (Arp2/3-branched networks at the leading edge; UI thread responsiveness, membrane tension regulation): fast, reversible, seconds-scale, driven by Rho GTPases. (2) Microtubule dynamics (GTP-tubulin polymerization toward the cell periphery; vesicle transport along kinesin/dynein tracks; Binder thread pool routing): directional, polarized, minutes-scale, driven by the centrosome MTOC. (3) Intermediate filaments (keratin, vimentin, nuclear lamina; structural backbone): far less dynamic than actin or microtubules — turnover is hours-scale rather than seconds — providing mechanical resilience that outlasts any single signaling event, analogous to the kernel's fixed memory layout and ABI stability. All three reorganize in response to force but on vastly different timescales; structural learning is not one mechanism but a three-layer system spanning seconds to hours.",
        scaleLabel: "Cellular",
...

  {
    ...
    phases: [
      ...
      },
      {
        ...
        description:
          "Receptor binding triggers allosteric conformational change: the intracellular domain adopts a new shape that recruits and activates a G-protein whose subtype is determined by the receptor itself — not all simultaneously. Gαs-coupled receptors activate adenylyl cyclase → cAMP produced → PKA activated → target proteins phosphorylated. Gαq-coupled receptors activate PLC → IP3 + DAG released → IP3 opens ER Ca²⁺ channels → CaM-kinase II cascade. Gαi-coupled receptors inhibit adenylyl cyclase, reducing cAMP. Incorrect ligands dissociate before triggering any cascade. The membrane's affect is discrimination followed by subtype-specific amplification — one receptor, one G-protein class, one downstream cascade. In Android: ISP pipeline processes the CaptureRequest (HAL configures the Image Signal Processor); Power HAL powerHint() type determines which downstream subsystem responds (CPU governor for INTERACTION, display for LAUNCH) — each hint type routes to exactly one cascade, not all simultaneously.",
        scaleLabel: "Molecular",
