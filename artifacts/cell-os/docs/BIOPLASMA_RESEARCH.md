...
**Action 8 — BP8 QED water coherence layer.** ✅ **IMPLEMENTED (SPA Phase 3, June 2026)** — `BP8_QED_WATER` exported as `status: "reserved"` in `bioplasmaPathways.ts`; `bioplasmaSignal()` returns immediately for BP8 (unconditional zero guard); not rendered in any UI panel. `bioplasmaBaseline` does not include BP8. Zero-weight annotation is live. Original action remains the activation criterion: do not activate until direct CD experimental evidence in biological systems (σ must reach 0.45 speculative tier minimum).
